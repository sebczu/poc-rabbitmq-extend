...
public class ExchangeTopicControllerTest extends RabbitmqPublisherApplicationTest {
  ...
  @Test
  public void shouldSendMessageIntoExchangeTopic() throws Exception {
    ...
    mockMvc.perform(post("/exchange/topic")
        .param("routing", "route")
  ...
  @Test
  public void shouldSendMessageIntoExchangeTopicWithRoutingAny() throws Exception {
    admin.declareQueue(queue1());
    admin.declareQueue(queue2());
    admin.declareBinding(BindingBuilder.bind(queue1()).to(exchange()).with("route.*"));
    admin.declareBinding(BindingBuilder.bind(queue2()).to(exchange()).with("route.1"));

    String message = "test";

    mockMvc.perform(post("/exchange/topic")
        .param("routing", "route.1")
        .param("message", message))
        .andDo(print())
        .andExpect(status().isOk())
        .andExpect(content().string(""));
  ...
  @Test
  public void shouldSendMessageIntoExchangeTopicWithRoutingSpecific() throws Exception {
    admin.declareQueue(queue1());
    admin.declareQueue(queue2());
    admin.declareBinding(BindingBuilder.bind(queue1()).to(exchange()).with("route.*"));
    admin.declareBinding(BindingBuilder.bind(queue2()).to(exchange()).with("route.1"));
